...
def _handle_claim_mutation(
    ...
    if command == "claim.release":
        # Release is proof-bound cleanup, not lease use.  The legacy SQLite
        # and PostgreSQL backends deliberately allow an owner to remove its
        # claim after expiry; requiring a live lease here would strand the row
        # because renew and handoff correctly refuse to revive it.
        _verify_claim_secret(
