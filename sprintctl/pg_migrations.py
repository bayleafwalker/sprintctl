...
# 17 adds run/evidence/session-note storage and the write-tool idempotency
    ...
    try:
        with store.conn.cursor() as cur:
            ...
            if state.version < 6:
                ...
                applied.append(6)
                state = SchemaState(version=6, row_count=1)
            if state.version < 7:
            ...
            if state.version < 9:
                ...
                applied.append(9)
                state = SchemaState(version=9, row_count=1)
            if state.version < 10:
                _pg._apply_schema_version_10(cur)
                cur.execute("UPDATE schema_version SET version = %s", (10,))
                applied.append(10)
                state = SchemaState(version=10, row_count=1)
            if state.version < 11:
            ...
            if state.version < 16:
                ...
                applied.append(16)
            if state.version < 17:
                _pg._apply_schema_version_17(cur)
                cur.execute("UPDATE schema_version SET version = %s", (17,))
                applied.append(17)
        store.conn.commit()
    except Exception:
        store.conn.rollback()
