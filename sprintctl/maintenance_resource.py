...
class MaintenanceResourceStore:
    ...
    def _ensure_schema(self) -> None:
        if self.postgres:
            statements = (
                "CREATE TABLE IF NOT EXISTS maintenance_resource (repo_id text NOT NULL, resource_ref text NOT NULL, capability_id text NOT NULL, recovery_floor bigint NOT NULL DEFAULT 0, current_position bigint NOT NULL DEFAULT 0, PRIMARY KEY(repo_id,resource_ref), UNIQUE(repo_id,capability_id))",
                "CREATE TABLE IF NOT EXISTS maintenance_resource_event (repo_id text NOT NULL, resource_ref text NOT NULL, position bigint NOT NULL, state text NOT NULL, updated_at timestamptz NOT NULL, PRIMARY KEY(repo_id,resource_ref,position))",
            )
        else:
            statements = (
                "CREATE TABLE IF NOT EXISTS maintenance_resource (resource_ref TEXT PRIMARY KEY, capability_id TEXT NOT NULL UNIQUE, recovery_floor INTEGER NOT NULL DEFAULT 0, current_position INTEGER NOT NULL DEFAULT 0)",
                "CREATE TABLE IF NOT EXISTS maintenance_resource_event (resource_ref TEXT NOT NULL, position INTEGER NOT NULL, state TEXT NOT NULL, updated_at TEXT NOT NULL, PRIMARY KEY(resource_ref,position))",
            )
    ...
    def ensure_reference(self, capability_id: str) -> str:
    ...
    def record_current(self, capability_id: str) -> str:
        resource_ref = self.ensure_reference(capability_id)
        ...
        if count > MAX_EVENTS:
            ...
            self._execute(f"UPDATE maintenance_resource SET recovery_floor=? WHERE {scope}resource_ref=?", (cutoff,) + values + (resource_ref,))
        self.conn.commit()
        return resource_ref

    def record_if_registered(self, capability_id: str) -> None:
        scope, values = self._scope()
        row = self._execute(f"SELECT 1 FROM maintenance_resource WHERE {scope}capability_id=?", values + (capability_id,)).fetchone()
        if row is not None:
            self.record_current(capability_id)
    ...
    def snapshot(self, resource_ref: str) -> dict[str, Any]:
        capability_id, _floor, position = self._binding(resource_ref)
        owner = self.lifecycle.get(capability_id)
        if owner is None:
            raise ResourceNotFound("resource not found")
        return {"schema_version": "resource-snapshot/v1", "reference": resource_ref, "revision": _revision(position), "cursor": _cursor(position), "terminal": owner["state"] in TERMINAL_STATES, "state": {"state": owner["state"], "not_before": _stamp(owner["not_before"]), "expires_at": _stamp(owner["expires_at"]), "updated_at": _stamp(owner["updated_at"])}}
    ...
    def reference_envelope(self, capability_id: str) -> dict[str, Any]:
        resource_ref = self.record_current(capability_id)
