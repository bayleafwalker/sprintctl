...
def write_session_note(store: PgStore, run_id: str, *, note: str) -> dict:
    try:
        with store.conn.cursor() as cur:
            cur.execute(
                "INSERT INTO session_note(repo_id, run_id, note) VALUES (%s, %s, %s) "
                "RETURNING note_id, note, created_at",
                (store.repo_id, run_id, note),
            )
            row = cur.fetchone()
        store.conn.commit()
    except Exception:
